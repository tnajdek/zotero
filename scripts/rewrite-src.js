...
function getRewrittenContent(content, fromPath) {
	content = content.replace(/<script(.*?)src="(.*?)"/ig, (match, p1, relPath) => {
		// console.log(relPath, '->', getAbsolutePath(relPath));
		const newPath = getAbsolutePath(relPath, fromPath);
		return newPath
			? `<script${p1}src="file://${newPath}"`
			: match;
	});
	...
	content = content.replace(/url\(["']?chrome:\/\/(.*?)["']?\)/ig, (match, relPath) => {
		// console.log(`chrome://${relPath}`, '=>', getAbsolutePath(`chrome://${relPath}`));
...
async function getRewriteSrc(source, options, signatures) {
	...
	while ((f = files.pop())) {
		...
		try {
			...
			const newContent = getRewrittenContent(content, path.dirname(f));
			// const newContent = getRewrittenContent(content);
