 ...
 * existing objects -- creation and permanent deletion are not undoable.
 */
Zotero.UndoHistory = {
	_undoStack: [],
	_redoStack: [],
	...
	_getObject(change) {
		switch (change.objectType) {
			case 'item':
				return Zotero.Items.get(change.id);
			case 'collection':
				return Zotero.Collections.get(change.id);
			case 'search':
				return Zotero.Searches.get(change.id);
			default:
				return null;
		}
	},
